...
impl FixedLayout {
    pub fn resolve(
        ...
        match self {
            ...
            FixedLayout::HorizontalSplit => {
                split_horizontal(full, count)
            }

            FixedLayout::VerticalSplit => {
                split_vertical(full, count)
            }

            FixedLayout::Grid { rows, cols } => {
                grid(full, count, *rows, *cols)
            }
            ...
            FixedLayout::Docked {
                ...
                let header_h =
                    header_height.unwrap_or(0.0).min(window_height);
                ...
                let left_w =
                    left_width.unwrap_or(0.0).min(window_width);
